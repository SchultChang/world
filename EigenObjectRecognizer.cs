      ...
      public EigenObjectRecognizer(Image<Gray, Byte>[] images, String[] labels, double eigenDistanceThreshold, ref MCvTermCriteria termCrit)
      {
         ...
         */
         ...
         _eigenDistanceThreshold = eigenDistanceThreshold;
            ...
            //// save file EigenImage.csv;
            //var filePathEigenImage = "eigenImage.csv";

            //using (StreamWriter writer = new StreamWriter(new FileStream(filePathEigenImage, FileMode.Create, FileAccess.Write)))
            //{
            //    foreach (Image<Gray, Single> image in _eigenImages)
            //    {
            //        var temp = new StringBuilder();
            //        for (int j = 0; j < _avgImage.Width; j++)
            //            for (int k = 0; k < _avgImage.Height; k++)
            //            {
            //                temp.Append(String.Format("{0}", image.Data[j, k, 0]));
            //                temp.Append(",");
            //            }
            //        temp.Append("0");
            ...
            ////save file AvgImage.csv
            //var filePathAvgImage = "AvgImage.csv";
            //using (StreamWriter writer = new StreamWriter(new FileStream(filePathAvgImage, FileMode.Create, FileAccess.Write)))
            //{
            //    var temp = new StringBuilder();
            //    for (int j = 0; j < _avgImage.Width; j++)
            //        for (int k = 0; k < _avgImage.Height; k++)
            //        {
            //            temp.Append(String.Format("{0}", _avgImage.Data[j, k, 0]));
            //            temp.Append(",");
            //        }
            //    temp.Append("0");
            //    writer.WriteLine(temp.ToString());
            ...
            //var filePathFeatureVector = "featureVector.csv";

            //using (StreamWriter writer = new StreamWriter(new FileStream(filePathFeatureVector, FileMode.Create, FileAccess.Write)))
            //{
            //    for (int i = 0; i < _eigenValues.Length; i++)
            //    {
            //        var temp = new StringBuilder();
            //        for (int j = 0; j < _eigenValues[i].Width; j++)
            //        {
            //            for (int k = 0; k < _eigenValues[i].Height; k++)
            //            {
            //                temp.Append(String.Format("{0}", _eigenValues[i].Data[k, j]));
            //                temp.Append(",");
            //            }
            //        }
            //        temp.Append("0");
            ...
            ////export file jpg AvgImage
            //_avgImage.ToBitmap().Save("AvgImage.jpg");
        }
